...
class core extends mysqli
{
	...
	function UpDate($tb = "", $name = "", $data = "", $where = "", $low = true)
	{
		if ($low)
			$tb = strtolower($tb);
		global $DBcon;
		if (is_array($name)) {
			...
			if (strpos($data, "*") !== false)
				$data = " $name " . str_replace("*", "+", $data);
			else
				$data = " '" . str_replace("'", "\'", $data) . "' ";
			// echo  " update " . $tb . " set `" . $name . "` = $data " . $where . '<br>';
			$sql =   $this->DBquery(" update " . $tb . " set `" . $name . "` = $data " . $where);
		}
		if ($sql) {
			return $sql;
		} else {
	...
	function getservices_images($param)
	{
		...
		if (array_key_exists('project_id', $param)) {
			...
			$condition .= " and project_id = $param[project_id]";
		}
		...
		while ($columns = $result->fetch_array(MYSQLI_ASSOC)) {
			...
			$img["order"] = $columns["order"];
			$img["active"] = $columns["active"];
			$img["project_id"] = $columns["project_id"];
	...
	function getsubservicesimages($pramsubservicesimages)
	{
		...
		if (!empty($pramsubservicesimages['sub_project_id'])) {
			$pramsubservicesimages['sub_project_id'] = (int)$pramsubservicesimages['sub_project_id'];
			$condition .= "and  sub_project_id = $pramsubservicesimages[sub_project_id]";
		}
		...
		while ($columns = $result->fetch_array(MYSQLI_ASSOC)) {
			$img["id"] = $columns["id"];
			$img["image"] = $columns["image"];
			$img["sub_project_id"] = $columns["sub_project_id"];
			$img["order"] = $columns["order"];
			$img["active"] = $columns["active"];
